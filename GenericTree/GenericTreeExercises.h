...
static void treeFactory(GenericTree<int>& tree) {
  ...
  std::cout << std::endl;
  std::cout << "------------------------------" << std::endl;
  std::cout << "TREE 3" << std::endl  << std::endl;

  tree.clear();
  tree.createRoot(4);
  auto L1 = tree.getRootPtr();
  auto L2_1 = L1->addChild(8);
  auto L2_2 = L1->addChild(15);
  auto L3_1 = L2_1->addChild(16);
  auto L3_2 = L2_1->addChild(23);
  auto L4_1 = L3_1->addChild(42);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  // If empty tree, return results empty vector 
  ...
  // Print starting tree, print starting results vector
  std::cout << tree << std::endl;

  // // Create current pointer and set to root node
  // TreeNode* cur = rootNodePtr;
  
  // // (Base Case) One node tree
  // if (cur->childrenPtrs.size()==0){
  ...
  std::queue<TreeNode*> nodeptrs;
  nodeptrs.push(rootNodePtr);
  
  while(!nodeptrs.empty()){
    
    // Make a copy of the top pointer on the stack, then pop it to decrease the stack
    TreeNode* cur = nodeptrs.front();
    nodeptrs.pop();
    
    if (cur==nullptr) {
      // if nullptr, move onto next item, no data to add to results vector
      continue;
    } 
    else{
