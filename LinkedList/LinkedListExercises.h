...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  std::cout << "----Making new node on heap---- "<< std::endl;
  //Create New node
  Node* newNode = new Node(newData);
  
  
  // If list is empty, set head and tail pointers to equal newNode pointer
  if (!head_){
  ...
  // item in the list.
  if (!(head_->next)) {
    std::cout << "----Single item list---- "<< std::endl;
  ...
  std::cout << "----Multi item list---- "<< std::endl;
  ...
  if ((newNode->data <= head_->data)) {
    ...
    while (cur->next) {
      ...
      lag = cur;
      cur = cur->next;
      if ((newNode->data >= lag->data) && (newNode->data <= cur->data)) {
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // and an int.)
    ...
    int elements_rem = lsize + rsize;

    
    //special cases first with empty lists
    if (left.size()==0 && right.size()==0){
      ...
      // Non empty lists of any size
      while (elements_rem !=0){
        ...
        if (left.front() <= right.front()){
          ...
          // Break conditions when one list has no elements remaining
          if (lsize==0){
              while (elements_rem !=0){
                x = right.front();
                right.popFront();
                merged.pushBack(x);
                rsize--;
                elements_rem = lsize+rsize;
                // std::cout << x << std::endl;
              }
            // Exit loop which fills in the remaining list items
            break;
          }
          else if (rsize==0){
              while (elements_rem !=0){
        ...
        // update elements remaining in main 
        elements_rem = lsize+rsize;
      }
    }
